...
export default function AdminCommercialAutonomy() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border bg-card p-4 space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap"><div><h2 className="text-sm font-black flex items-center gap-2"><ShieldCheck size={14}/> Founder-approved policies</h2><p className="text-[11px] text-muted-foreground mt-1">No autonomous external send is allowed without an active versioned policy.</p></div><div className="flex gap-2"><button onClick={() => createDraft("merchant_acquisition")} disabled={busy} className="h-8 px-3 rounded-lg border text-xs font-bold">New acquisition policy</button><button onClick={() => createDraft("provider_negotiation")} disabled={busy} className="h-8 px-3 rounded-lg border text-xs font-bold">New negotiation policy</button></div></div>
