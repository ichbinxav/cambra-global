  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Link to="/admin/inbox" className="rounded-2xl border border-white/10 bg-card/70 p-5 hover:border-cyan-400/30 transition-colors"><ShieldAlert size={18} className="text-cyan-300" /><h3 className="font-black mt-3">Approve and reject</h3><p className="text-xs text-muted-foreground mt-1">Canonical Founder Inbox · {founder?.approvals?.length || goLive?.runtime?.pending_approval_count || goLive?.runtime?.pending_approvals?.length || 0} pending</p></Link>
        ...
        <div className="rounded-2xl border border-white/10 bg-card/70 p-5"><ShieldAlert size={18} className="text-amber-300" /><h3 className="font-black mt-3">Critical exceptions</h3><p className="text-xs text-muted-foreground mt-1">{goLive?.runtime?.open_incident_count || goLive?.runtime?.open_incidents?.length || founder?.critical_exceptions?.length || 0} open incidents · never hidden by a composite score</p></div>
      </div>
    </div>
  );
}
