...
const LOCAL_COPY = {
  ...
  es: {
    ...
    "Founder Control is unavailable":"Founder Control no está disponible", "No permissive state is inferred. Material effects remain fail-closed until the canonical control snapshot can be read.":"No se infiere ningún estado permisivo. Los efectos materiales permanecen cerrados por defecto hasta poder leer la instantánea de control canónica.", "Retry":"Reintentar", "Founder Control":"Founder Control", "One calm view of what CAMBRA can actually do":"Una vista clara de lo que CAMBRA puede hacer realmente", "Authority, operating modes, dependencies, economic limits and material decisions. Connected never means authorized, and a proposed action never means executed.":"Autoridad, modos operativos, dependencias, límites económicos y decisiones materiales. Conectado nunca significa autorizado, y una acción propuesta nunca significa ejecutada.", "Refresh":"Actualizar",
...
export default function AdminFounderControl() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const data = await invoke("getFounderControlCenter", {});
      setSnapshot(data);
  ...
  if (!snapshot) return <div className="rounded-2xl border border-rose-400/25 bg-rose-400/10 p-6"><h1 className="text-xl font-black">{tr("Founder Control is unavailable")}</h1><p className="mt-2 text-sm text-muted-foreground">{tr("No permissive state is inferred. Material effects remain fail-closed until the canonical control snapshot can be read.")}</p><button onClick={() => load()} className="mt-4 rounded-xl border px-4 py-2 text-sm font-bold">{tr("Retry")}</button></div>;
