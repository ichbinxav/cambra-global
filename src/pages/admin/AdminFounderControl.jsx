...
const LOCAL_COPY = {
  fr: {
    ...
    "Cost safety pause":"Pause de sécurité des coûts", "Paid operations are paused because a cost allowance was exhausted. No email is being sent.":"Les opérations payantes sont suspendues parce qu’une enveloppe de coûts a été épuisée. Aucun e-mail n’est envoyé.", "Review and clear":"Examiner et lever", "Clear cost safety pause":"Lever la pause de sécurité des coûts", "This clears only the cost emergency stop. Outbound and every sending profile remain paused until a separate fresh preflight and explicit canary activation.":"Cette action lève uniquement l’arrêt d’urgence des coûts. L’outbound et chaque profil d’envoi restent en pause jusqu’à un nouveau contrôle préalable distinct et une activation explicite du pilote.", "Budget version:":"Version du budget :", "Trigger:":"Déclencheur :", "Activated:":"Activée :", "Why it is safe to clear this cost pause":"Pourquoi cette pause de coûts peut être levée en toute sécurité", "Cost safety pause cleared":"Pause de sécurité des coûts levée", "Clear pause":"Lever la pause"
  },
  es: {
    ...
    "Cost safety pause":"Pausa de seguridad de costes", "Paid operations are paused because a cost allowance was exhausted. No email is being sent.":"Las operaciones de pago están pausadas porque se agotó una partida de costes. No se está enviando ningún correo.", "Review and clear":"Revisar y resolver", "Clear cost safety pause":"Resolver la pausa de seguridad de costes", "This clears only the cost emergency stop. Outbound and every sending profile remain paused until a separate fresh preflight and explicit canary activation.":"Esto elimina únicamente la parada de emergencia de costes. El outbound y todos los perfiles de envío seguirán pausados hasta una comprobación previa nueva e independiente y la activación explícita del piloto.", "Budget version:":"Versión del presupuesto:", "Trigger:":"Motivo:", "Activated:":"Activada:", "Why it is safe to clear this cost pause":"Por qué es seguro resolver ahora esta pausa de costes", "Cost safety pause cleared":"Pausa de seguridad de costes resuelta", "Clear pause":"Resolver pausa"
  },
};
...
export default function AdminFounderControl() {
  ...
  const openCostStopClear = () => {
    setReason("");
    setModal({ kind:"cost-stop-clear", control:budget });
  };
  const confirmCostStopClear = () => run("cost-stop-clear", async () => {
    const result = await invoke("outboundControlAdmin", {
      action:"clear_cost_emergency_stop",
      confirmation:"CLEAR_COST_EMERGENCY_STOP",
      reason,
    });
    setModal(null);
    return result;
  }, tr("Cost safety pause cleared"));
  ...
  return (
    <div className="space-y-5 pb-16">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
        ...
        <section className="rounded-2xl border border-white/[.08] bg-card/70 p-5">
          <div className="flex items-center justify-between gap-3"><div><h2 className="font-black">{tr("Hard economic limits")}</h2><p className="mt-1 text-xs text-muted-foreground">{tr("Agents cannot raise these limits.")}</p></div><button onClick={openBudget} disabled={!!busy || budget?.emergency_stop_active === true} className="inline-flex h-9 items-center gap-1.5 rounded-lg border px-3 text-xs font-bold disabled:opacity-35"><SlidersHorizontal size={13} />{tr("Change")}</button></div>
          {budget?.emergency_stop_active === true && <div className="mt-4 rounded-xl border border-rose-400/25 bg-rose-400/[.08] p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black text-rose-200">{tr("Cost safety pause")}</p><p className="mt-1 text-[10px] leading-4 text-rose-100/70">{tr("Paid operations are paused because a cost allowance was exhausted. No email is being sent.")}</p></div><StatusPill value="EMERGENCY_STOPPED" /></div><button onClick={openCostStopClear} disabled={!!busy} className="mt-3 h-9 w-full rounded-lg bg-rose-500 text-xs font-black text-white disabled:opacity-40">{tr("Review and clear")}</button></div>}
      ...
      <Dialog open={!!modal && modal.kind !== "resume" && modal.kind !== "budget"} onOpenChange={open => !open && setModal(null)}>
        <DialogContent className="max-h-[88vh] overflow-y-auto rounded-3xl sm:max-w-xl">
          <DialogHeader><DialogTitle>{modal?.kind === "stop" ? tr("Confirm Global Emergency Stop") : modal?.kind === "approval" ? tr("{decision} material action", { decision:label(modal.decision, tr) }) : modal?.kind === "cost-stop-clear" ? tr("Clear cost safety pause") : tr("Start controlled canary")}</DialogTitle><DialogDescription>{modal?.kind === "stop" ? tr("Review the real impact. Local containment remains effective even if a provider pause needs retry.") : modal?.kind === "approval" ? tr("The preview is bound to current state and will be revalidated before execution.") : modal?.kind === "cost-stop-clear" ? tr("This clears only the cost emergency stop. Outbound and every sending profile remain paused until a separate fresh preflight and explicit canary activation.") : tr("Outbound changes from OFF only after this explicit confirmation and a fresh matching preflight.")}</DialogDescription></DialogHeader>
          ...
          {modal?.kind === "cost-stop-clear" && <div className="space-y-2 rounded-xl border border-rose-400/20 bg-rose-400/[.05] p-4 text-xs"><p><strong>{tr("Budget version:")}</strong> {modal.control?.version || "—"}</p><p><strong>{tr("Trigger:")}</strong> {label(modal.control?.emergency_stop_reason, tr)}</p><p><strong>{tr("Activated:")}</strong> {when(modal.control?.emergency_stop_activated_at, locale)}</p></div>}
          ...
          <DialogFooter><button onClick={() => setModal(null)} className="h-10 rounded-xl border px-4 text-xs font-bold">{tr("Cancel")}</button><button onClick={modal?.kind === "stop" ? confirmStop : modal?.kind === "approval" ? confirmApproval : modal?.kind === "cost-stop-clear" ? confirmCostStopClear : confirmCanary} disabled={!!busy || (modal?.kind !== "canary" && reason.trim().length < 3)} className={`h-10 rounded-xl px-4 text-xs font-black disabled:opacity-40 ${modal?.kind === "stop" || modal?.kind === "cost-stop-clear" || (modal?.kind === "approval" && modal?.decision === "reject") ? "bg-rose-500 text-white" : "bg-foreground text-background"}`}>{busy ? tr("Applying…") : modal?.kind === "cost-stop-clear" ? tr("Clear pause") : tr("Confirm with fresh preview")}</button></DialogFooter>
        </DialogContent>
      </Dialog>
