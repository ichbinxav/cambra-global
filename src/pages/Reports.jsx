...
import {
  ArrowRight, TrendingUp, ArrowUpRight, CheckCircle2, Circle, AlertCircle, Sparkles,
} from "lucide-react";
    ...
    <div className="animate-fade-up">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-40">
          <div className="text-2xl text-muted-foreground/25" style={{ animation: "spin 4s linear infinite" }}>✱</div>
          <style>{`@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
        </div>
      ) : results.length === 0 ? (
        <div
          className="relative rounded-2xl border border-white/[0.08] overflow-hidden p-12 sm:p-16 text-center animate-fade-up"
          style={{
            background:
        ...
        <>
          ...
          {/* Chart */}
          {chartData.length > 0 && (
            <div className="cambra-card p-7 mb-6 animate-fade-up">
              <div className="relative">
          ...
          {/* History list */}
          <div className="cambra-card overflow-hidden animate-fade-up">
            ...
            <div className="divide-y divide-white/[0.06] relative">
              {results.map((r, i) => {
                ...
                const scoreColor =
                  score >= 75
                    ? "text-[#52EBA4] bg-[#52EBA4]/10 border-[#52EBA4]/25"
                    : score >= 50
                    ? "text-[#7BD9F0] bg-[#7BD9F0]/10 border-[#7BD9F0]/25"
                    : score >= 30
                    ? "text-[#FFB05A] bg-[#FFB05A]/10 border-[#FFB05A]/25"
                    : "text-[#FF7A6E] bg-[#FF7A6E]/10 border-[#FF7A6E]/25";
                return (
                      ...
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-white truncate">
                            {format(new Date(r.created_date), "MMMM d, yyyy")}
                          </p>
                          <p className="text-[11px] text-white/45 font-mono mt-0.5">
                            {format(new Date(r.created_date), "HH:mm")} · scan complete
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 sm:gap-4 shrink-0">
                        ...
                        <div className="text-right">
                          <p className="text-sm font-black tabular-nums tracking-tight text-white">
                            €{r.total_savings?.toLocaleString()}<span className="text-white/40 font-normal">/yr</span>
                          </p>
