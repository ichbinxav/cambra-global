...
Large backups use the authenticated `cambra-dr-sharded-stage-v1` layout. Each small encrypted part is downloaded and hash-verified immediately after upload; the final index and snapshot are encrypted descriptors whose signed part inventory covers the complete catalog. Restore and incremental reads open only the parts that contain the required entities, so no finalization step materializes the complete catalog in memory. A failed finalization preserves the encrypted pending operation and verified parts for idempotent continuation; deletion of an unmanifested backup remains a separate confirmation-gated action. A complete legacy 24-entity pending operation can be deterministically repartitioned under the same backup identity, and superseded legacy parts are removed only after the new manifest is published and verified.
