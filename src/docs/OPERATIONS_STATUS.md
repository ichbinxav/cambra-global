...
The declared frontend TypeScript surface is at **0 errors** with `allowJs`/`checkJs` active. Its exact scope is `src/components`, `src/pages`, `src/api` and `src/Layout.jsx`, excluding test files, Vite plugins and generated Radix wrappers. It is not a full-repository claim. The independently zero-error backend perimeter covers economic/ECL, webhooks, outbound, emergency control, scheduler guard, public error handling and admin bootstrap; full legacy-backend strictness remains incremental debt. The sanctioned frontend baseline remains 0, so any future diagnostic in that declared scope is a release regression.
