...
**Censo:** 2026-07-24 (actualizado 2026-08-11 con P1 Europe + P2 Provider Intelligence + P3 European Rate Intelligence + ECL P8/P9 + P6–P13 European Operating Architecture + pre-GO CANARY, Founder meeting seal y v0.95 P7/P8 Instantly transport) · **Total: 300 funciones desplegables** · Generado por análisis estático de `base44/functions/*/entry.ts` + índice de callers en `src/` + automatizaciones registradas en plataforma. **Este documento es SOLO el mapa** — no se borró ni archivó nada. Es la base del segundo barrido PURGE-2 (15-ago).
...
| instantlyProviderAdmin | B · admin · alojada también en outboundControlAdmin | CommercialProviderState, OutboundSendingProfile, OutboundControl, CostUsageEvent, OperationalLog | Estado/diagnóstico API v2, campaña DRAFT y webhook autenticado con confirmación; nunca autoactiva, nunca devuelve secrets y pausa remota sin depender del presupuesto |
...
| goLiveControlAdmin | B · admin | RuntimeGateEvidence, EmergencyControl, CostBudgetControl, SchedulerRun, OperationalLog | Command plane fail-closed para DNS SPF/DKIM/DMARC, schedulers, supresión, bucle observable, presupuestos, simulacro de parada/reanudación y prueba de control Founder; nunca certifica evidencia local como runtime real |
