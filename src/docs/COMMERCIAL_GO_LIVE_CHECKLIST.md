...
7. **Verify DNS and credentials.** Run the real-runtime verifier. Every configured profile, including paused reserve domains, must pass SPF, every explicit DKIM selector and DMARC. Resend requires a valid `RESEND_API_KEY` and a webhook created on the canonical `resendInboundWebhook` URL; `resend_register_webhook` requires `REGISTER_CAMBRA_RESEND_WEBHOOK`, returns the provider signing secret once to the authenticated Founder for immediate storage as `RESEND_WEBHOOK_SECRET`, and becomes non-disclosing once the vault value exists. For Instantly, campaign/webhook transport credentials are required only for profiles that are active, warming or explicitly prepared with an external campaign; inventory-only paused reserves never become a shortcut around DNS and cannot block a correctly isolated one-profile canary. Rotation and provider delivery testing remain explicit admin actions. Outlook requires a live connector token.
