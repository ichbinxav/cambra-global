...
describe('CAMBRA disaster recovery hard gate',()=>{
  ...
  it('stages the complete catalog through durable bounded resumable invocations before sealing one snapshot',()=>{
    ...
    expect(runtime).toContain('const BACKUP_STAGE_CHUNKS_PER_INVOCATION=1');
    ...
    expect(runtime).toContain("event:'disaster_recovery_backup_finalization_stage_loaded'");
    ...
    expect(runtime).toContain('row.records=[];row.tombstones=[];row.index={}');
    expect(runtime).toContain('rows.length=0');
    ...
    expect(read('src/pages/admin/AdminMaintenance.jsx')).toContain('pass<16');
    ...
    expect(runtime).toContain('loadBackupStage(storage,key,operation.artifacts[chunkIndex],coordinates)');
