...
describe('CAMBRA disaster recovery hard gate',()=>{
  ...
  it('stages the complete catalog through remotely verified shards before sealing one snapshot',()=>{
    const runtime=read('base44/shared/disasterRecoveryRuntime.ts');
    expect(runtime).toContain('const BACKUP_ENTITY_BATCH_SIZE=2');
    expect(runtime).toContain('const LEGACY_BACKUP_ENTITY_BATCH_SIZE=24');
    ...
    expect(runtime).toContain("const BACKUP_OPERATION_VERSION='cambra-dr-backup-operation-v2'");
    expect(runtime).toContain("const LEGACY_BACKUP_OPERATION_VERSION='cambra-dr-backup-operation-v1'");
    expect(runtime).toContain("const BACKUP_DATA_LAYOUT='cambra-dr-sharded-stage-v1'");
    ...
    expect(runtime).not.toContain('mapLimitDrained(pairs,BACKUP_STAGE_READ_CONCURRENCY');
    expect(runtime).toContain('for(let chunkIndex=0;chunkIndex<operation.artifacts.length;chunkIndex++)');
    expect(runtime).toContain("event:'disaster_recovery_backup_descriptors_uploaded'");
    expect(runtime).toContain("hash=createHash('sha256')");
    expect(runtime).toContain("stream=new CompressionStream('gzip')");
    expect(runtime).toContain('for(const chunk of jsonValueChunks(value))await write(chunk)');
    expect(runtime).not.toContain('encoder.encode(JSON.stringify(value))');
    expect(runtime.indexOf('const indexArtifact=await')).toBeLessThan(runtime.indexOf('snapshotArtifact=await'));
    expect(runtime).toContain('allFiles.clear()');
    expect(runtime).toContain('data_layout:BACKUP_DATA_LAYOUT,parts_summary:partsSummary,parts');
    expect(runtime).toContain('function validateShardedParts(manifest:any,descriptor:any)');
    expect(runtime).toContain('async function loadShardedCheckpointIndexes');
    expect(runtime).toContain('attachment_items:attachments');
    expect(runtime).toContain("const snapshot=await loadVerifiedJsonArtifact(storage,manifest.snapshot,key,'snapshot')");
    ...
    expect(runtime).not.toContain("invokeInternal(base44,'getMaintenanceCenter'");
    expect(runtime).not.toContain("invokeInternal(base44,'maintenanceEngine'");
    expect(runtime).toContain('chunk=await executeBackupChunk(req,service,{backup_id:operation.backup_id');
    expect(runtime).toContain("event:'disaster_recovery_backup_chunk_started'");
    ...
    expect(maintenanceRead).not.toContain('dr_backup_chunk');
    expect(read('src/pages/admin/AdminMaintenance.jsx')).toContain('pass<64');
    ...
    expect(runtime).toContain('remote=await storage.download(coordinates.stagePath)');
    expect(runtime).toContain('summary,remote_verified:true');
    expect(runtime).toContain('loadBackupStage(storage,key,validated.parts[chunkIndex],coordinates)');
    expect(runtime).toContain('exactTextArray(observedEntityNames,DISASTER_RECOVERY_ENTITY_CATALOG)');
    expect(runtime).toContain('await cleanupSupersededBackupStageArtifacts(storage,operation)');
    expect(runtime).not.toContain('await cleanupBackupStage(storage,operation.retention_tier,operation.backup_id)');
    ...
    expect(runtime).not.toContain('removeUnmanifestedBackup(storage,operation.retention_tier,operation.backup_id,key)');
    expect(runtime).toContain("logDrFailure('disaster_recovery_backup_operation_paused',error)");
