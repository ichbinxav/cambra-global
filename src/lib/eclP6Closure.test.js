...
describe("ECL P6 — Economic Execution & Reconciliation", () => {
  ...
  it("binds a Stripe invoice to the exact frozen local identity and cents", () => {
    ...
    expect(
      validateStripeInvoiceBinding(inv, remoteInvoice({ total: 2999 })).reasons,
    ).toContain("stripe_total_mismatch");
    expect(
      validateStripeInvoiceBinding(
        inv,
        remoteInvoice({ customer: "cus_other" }),
      ).reasons,
    ).toContain("stripe_customer_mismatch");
    expect(
      validateStripeInvoiceBinding(
        inv,
        remoteInvoice({
          metadata: { ...remoteInvoice().metadata, local_invoice_id: "other" },
        }),
      ).reasons,
    ).toContain("stripe_metadata_local_invoice_mismatch");
  });
  ...
  it("projects current Stripe state without letting late invoice events erase dispute/refund state", () => {
    expect(
      stripeStatusProjection(
        localInvoice(),
        remoteInvoice({ status: "paid", amount_paid: 3000, amount_due: 0 }),
      ).targetStatus,
    ).toBe("paid");
    expect(
      stripeStatusProjection(
        localInvoice({ status: "disputed" }),
        remoteInvoice({ status: "paid", amount_paid: 3000, amount_due: 0 }),
      ).targetStatus,
    ).toBe("disputed");
    expect(
      stripeStatusProjection(
        localInvoice({ status: "refunded" }),
        remoteInvoice({ status: "open" }),
      ).targetStatus,
    ).toBe("refunded");
  });

  it("CAS-claims one local draft per report under a concurrent race", async () => {
    const rows = [];
    let seq = 0;
    const report = {
      id: "r_1",
    ...
    const svc = {
      entities: {
        MonthlySavingsReport: {
          filter: async ({ id }) => id === report.id ? [{ ...report }] : [],
          updateMany: async (filter, update) => {
            if (
              filter.id !== report.id ||
              filter.billing_eligibility_status !==
                report.billing_eligibility_status
            ) return { updated: 0 };
            if (
              filter.invoice_id !== undefined &&
              String(filter.invoice_id) !== String(report.invoice_id)
            ) return { updated: 0 };
            if (
              filter.invoice_claim_token !== undefined &&
              String(filter.invoice_claim_token) !==
                String(report.invoice_claim_token)
            ) return { updated: 0 };
        ...
        Invoice: {
          filter: async (filter) =>
            rows.filter((r) =>
              ...
              (!filter.execution_key ||
                r.execution_key === filter.execution_key)
            ),
          create: async (record) => {
            const row = {
              id: `i${++seq}`,
              created_date: `2026-08-09T00:00:0${seq}.000Z`,
              ...record,
            };
            rows.push(row);
            return row;
    ...
    const [first, second] = await Promise.all([
      claimRecoverInvoiceDraft(svc, key, {
        status: "draft",
        monthly_savings_report_id: "r_1",
      }),
      claimRecoverInvoiceDraft(svc, key, {
    ...
    const winner = [first, second].find((claim) => claim.acquired);
    const loser = [first, second].find((claim) => !claim.acquired);
    expect(winner).toBeTruthy();
    expect(loser.in_progress).toBe(true);
    expect(winner.invoice.id).toBe(loser.invoice.id);
    expect(rows).toHaveLength(1);
    await expect(
      finalizeRecoverInvoiceClaim(svc, {
        ...loser,
        acquired: true,
        claim_token: "forged",
      }, { status: "invoiced" }),
    )
      .rejects.toThrow("recover_invoice_claim_lost_before_finalize");
    expect(report.billing_eligibility_status).toBe("invoice_claimed");
    await finalizeRecoverInvoiceClaim(svc, winner, {
      status: "invoiced",
      verification_status: "invoiced",
    });
    expect(report.billing_eligibility_status).toBe("invoiced");
  });
  ...
  it("issuer claims locally before the first Stripe POST and keeps Stripe idempotency keys", () => {
    ...
    expect(ISSUER).toContain("convergeRecoverInvoiceIssuance");
  });
  ...
  it("webhook verifies signature, dedupes authoritatively, then GETs current Stripe state before local update", () => {
    ...
    const dedupe = WEBHOOK.indexOf(
      "const duplicates = await readBoundedPaymentEvents",
    );
    const remoteGetOffset = WEBHOOK.slice(dedupe).search(
      /stripeRequest\(\s*mode,\s*["']GET["']/,
    );
    const remoteGet = remoteGetOffset < 0 ? -1 : dedupe + remoteGetOffset;
    const binding = WEBHOOK.indexOf(
      "const binding = validateStripeInvoiceBinding",
    );
    const update = WEBHOOK.indexOf(
    ...
    expect(WEBHOOK).not.toMatch(
      /PaymentEvent\.filter\([\s\S]{0,220}\.catch\(\(\) => \[\]\)/,
    );
    expect(WEBHOOK).toContain("reconciliation_mismatch");
  });

  it("scheduled reconciler is Stripe-read-only and runs every 15 minutes", () => {
    expect(RECONCILER).toMatch(/stripeRequest\(\s*mode,\s*["']GET["']/);
    expect(RECONCILER).not.toMatch(/stripeRequest\(\s*mode,\s*["']POST["']/);
  ...
  it("blocks every local/manual second source of truth for Recover Stripe invoices", () => {
    for (const src of [MANUAL, OVERRIDE]) {
      ...
      expect(src).toMatch(/use:\s*["']reconcileRecoverBilling["']/);
    }
    expect(OVERRIDE).toContain(
      "finalized_invoice_amounts_are_immutable_use_credit_note_or_corrective_invoice",
    );
    expect(LINK).toContain(
      "recover_invoice_already_has_stripe_payment_surface",
    );
    const linkGuard = LINK.indexOf(
      "recover_invoice_already_has_stripe_payment_surface",
    );
    const stripeKeyRead = LINK.search(
      /Deno\.env\.get\(["']STRIPE_API_KEY["']\)/,
    );
    expect(linkGuard).toBeGreaterThan(-1);
    expect(stripeKeyRead).toBeGreaterThan(-1);
    expect(linkGuard).toBeLessThan(stripeKeyRead);
  });
