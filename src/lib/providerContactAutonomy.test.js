...
describe('provider contact autonomy',()=>{
 ...
 it('redirected contact gets its own idempotency key',()=>{const s=read('base44/functions/providerNegotiationAgent/entry.ts');expect(s).toContain("provider-initial:${c.id}:${String(thread.counterparty_email||'').toLowerCase()}")});
});
