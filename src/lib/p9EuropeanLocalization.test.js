...
describe('P9 European localization and productization', () => {
  it('separates 33 markets from the implemented product locales and currencies', () => {
    ...
    expect(PRODUCT_LOCALES.map((x) => x.locale)).toEqual(['en-GB', 'fr-FR', 'es-ES', 'de-DE', 'it-IT', 'pl-PL']);
  ...
  it('uses market policy before browser and labels non-native fallback honestly', () => {
    ...
    // market whose native language is still pending (Portugal until `pt` lands).
    expect(resolveLocale({ market_code: 'PT' })).toMatchObject({ locale: 'en-GB', source: 'market_default', fallback_used: true });
    expect(localizationReadiness('PT')).toMatchObject({ status: 'LIMITED', launch_gate: 'BLOCK_FULL_LAUNCH' });
  });
