...
describe('consent-gated product analytics',()=>{
  beforeEach(()=>{track.mockClear();globalThis.localStorage={getItem:vi.fn(()=>null)};});
  ...
  it('tracks only allowlisted events after consent',()=>{globalThis.localStorage.getItem=vi.fn(()=>JSON.stringify({analytics:true}));expect(trackProductEvent('analysis_started',{source:'analyzer'})).toBe(true);expect(track).toHaveBeenCalledOnce();expect(trackProductEvent('invented_event')).toBe(false)});
