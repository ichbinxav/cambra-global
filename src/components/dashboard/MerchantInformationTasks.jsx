...
export default function MerchantInformationTasks({ lang = "en" }) {
  ...
  return <div className="space-y-3" data-testid="merchant-information-tasks">{items.map(item=><div key={item.id} className="rounded-[22px] p-5" style={{background:"linear-gradient(145deg,#FFFDF6,#FFFFFF)",border:"1px solid rgba(202,138,4,.22)",boxShadow:"0 18px 52px -42px rgba(98,71,8,.55)"}}>
    <div className="flex gap-3 items-start"><div className="h-9 w-9 rounded-xl flex items-center justify-center shrink-0" style={{background:"#FFF4C7",border:"1px solid rgba(202,138,4,.20)"}}><AlertCircle size={16} className="text-amber-700"/></div><div className="min-w-0 flex-1">
      <p className="text-[10px] uppercase tracking-[.18em] font-bold text-amber-700 mb-1">{t.eyebrow}</p><h3 className="font-bold text-sm text-[#151329]">{item.what_needed}</h3><p className="text-xs mt-1 leading-relaxed text-[#6C6879]">{item.why_needed}</p><p className="text-xs mt-3 leading-relaxed text-[#343145]">{item.merchant_action}</p><p className="text-[11px] mt-1 text-[#8A8797]">{item.estimated_effort} · {item.what_happens_next}</p>
      <div className="flex flex-col sm:flex-row gap-2 mt-4"><input value={values[item.id]||""} onChange={e=>setValues(v=>({...v,[item.id]:e.target.value}))} placeholder={t.placeholder} className="flex-1 h-10 rounded-xl px-3 bg-white border border-[#DCD9E8] text-sm text-[#151329] outline-none focus:border-[#5B4CF5]"/><button type="button" onClick={()=>answer(item,item.information_type==="provider_contact"?"email":"text")} disabled={busy===item.id||!String(values[item.id]||"").trim()} className="h-10 px-4 rounded-xl bg-[#151329] text-white text-xs font-bold disabled:opacity-40">{busy===item.id?<Loader2 size={14} className="animate-spin"/>:t.send}</button><button type="button" onClick={()=>answer(item,"dont_know")} disabled={busy===item.id} className="h-10 px-4 rounded-xl border border-[#DCD9E8] text-[#575366] text-xs font-semibold hover:border-[#AAA3D8] hover:text-[#151329]">{t.unknown}</button></div>
      {notice?.id===item.id&&<p className={`text-xs mt-3 ${notice.ok?"text-emerald-700":"text-red-700"}`}>{notice.ok?<><Check size={12} className="inline mr-1"/>{t.done}</>:t.error}</p>}
    </div></div>
  </div>)}</div>;
}
