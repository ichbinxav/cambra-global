    ...
    {rows.length ? <div className="space-y-2">{rows.map(t=><div key={t.id} className="rounded-lg border border-border p-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex gap-1 shrink-0">
          ...
          {!['done','canceled'].includes(t.status)&&<button onClick={()=>move(t,'done')} disabled={!!busy||!(note[t.id]||'').trim()} title={(note[t.id]||'').trim()?'Complete with evidence note':'Add an evidence note before completing'} className="h-7 px-2 rounded bg-foreground text-background text-[10px] font-bold disabled:opacity-50">Complete</button>}
        </div>
      </div>
