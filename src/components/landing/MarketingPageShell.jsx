 ...
 *   - continuous editorial gradient background
 *   - fixed ambient grid noise
 *   - dark MarketingNavbar
 *   - optional unified hero (eyebrow pill + huge gradient headline + subtitle)
 *
 * Children render below the hero (or directly under the navbar if no hero is given).
 */
        ...
        <div className={`relative ${maxWidth} mx-auto px-6 sm:px-10`}>
          {title && (
            <header className={`mb-12 sm:mb-16 ${isCentered ? "text-center" : "text-left"}`}>
              {eyebrow && (
                <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 mb-6 animate-fade-up`}
                  style={{
              ...
              {heroActions && (
                <div className={`mt-8 animate-fade-up ${isCentered ? "flex justify-center" : ""}`} style={{ animationDelay: "350ms" }}>
                  {heroActions}
                </div>
              )}
            </header>
