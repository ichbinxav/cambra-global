...
export async function buildCommercialOperatingSystem(service:any){
  const [radar,policies,campaigns,profiles,controls,threads,messages,strategies,tasks,approvals,questions,providerStates,schedulerRuns]=await Promise.all([
    ...
    service.entities.CommercialPolicy.filter({engine:'merchant_acquisition'},'-updated_date',50).catch(()=>[]),
    service.entities.CommercialCampaign.list('-updated_at',50).catch(()=>[]),
    ...
    service.entities.CommunicationThread.filter({engine:'merchant_acquisition'},'-last_message_at',100).catch(()=>[]),
    service.entities.CommunicationMessage.list('-created_date',500).catch(()=>[]),
    service.entities.CommercialStrategy.list('-created_at',200).catch(()=>[]),
    service.entities.AgentTask.list('-created_date',200).catch(()=>[]),
    service.entities.Approval.filter({status:'pending'},'-created_date',100).catch(()=>[]),
    service.entities.AgentQuestion.filter({status:'pending'},'-created_date',100).catch(()=>[]),
  ...
  return{
    ...
    safety:{outbound_locked:control.acquisition_enabled!==true,control:compactControl,explicit_pilot_required:true,external_send_performed:false},
