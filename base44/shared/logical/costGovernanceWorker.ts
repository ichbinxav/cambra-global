...
export async function handleCostGovernanceWorker(req: Request) {
  ...
  try {
    ...
    if (snapshot.validation.ok && hardStopPct > 0 && maximumPct >= hardStopPct) {
      ...
      const old = await svc.entities.AutonomyIncident.filter({ dedupe_key:'cost-budget-anomaly-warning', status:'open' }, '-last_seen_at', 2).catch((error:any)=>safeBestEffort(error,{operation:'costGovernanceWorker.resolveWarning',fallback:[],severity:'secondary'}));
      if (old.length === 1) {
        const priorActions = Array.isArray(old[0].actions_taken_json) ? old[0].actions_taken_json : [];
        await svc.entities.AutonomyIncident.update(old[0].id, { status:'resolved', workflow_state:'resolved', resolved_at:now, actions_taken_json:[...priorActions,{at:now,action:'cost_utilization_returned_below_warning_threshold',verified:true,maximum_utilization_pct:maximumPct,warning_threshold_pct:warningPct}].slice(-100), recovery_json:{...(old[0].recovery_json||{}),verified:true,reason:'current_cost_snapshot_below_warning_threshold',maximum_utilization_pct:maximumPct,warning_threshold_pct:warningPct,budget_version:snapshot.control?.version||null} });
      } else if (old.length > 1) {
        safeBestEffort(new Error('cost_warning_incident_ambiguous'),{operation:'costGovernanceWorker.resolveWarning',fallback:null,severity:'critical'});
      }
    }
