...
} from './providerOpportunityProtection.ts';

const text = (value: unknown, max = 500) => String(value || '').trim().slice(0, max);
const addDays = (iso: string, days: number) => new Date(Date.parse(iso) + days * 86400000).toISOString();

export async function handleProviderOpportunityRegistrationAction(user: any, body: any, svc: any) {
  try {
    ...
    if (action === 'prepare') {
      ...
      const registration = await svc.entities.ProviderOpportunityRegistration.create({
        ...
        prepared_by: actor,
        updated_by: actor,
        audit_json: { events: [{ action: 'PREPARED', at: now, by: actor }] },
      });
    ...
    console.error('providerOpportunityRegistration failed', error);
