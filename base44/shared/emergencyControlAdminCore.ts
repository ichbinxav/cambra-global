// FCTRL-J (2026-08-16) — the full emergencyControlAdmin handler, extracted from
// base44/functions/emergencyControlAdmin/entry.ts verbatim so behavior tests can
// invoke it without Deno.serve. entry.ts remains the only trust boundary; this
// module never reads the request or environment directly. The `deps` parameter
// exists only for test injection — every default is the real implementation.
import { buildEmergencyStopPreview as realBuildEmergencyStopPreview, evaluateSafeResume as realEvaluateSafeResume, FOUNDER_CONTROL_V2_VERSION, RESUMABLE_CAPABILITIES } from './founderControlV2.ts';
import { pauseAllInstantlyCampaigns as realPauseAllInstantlyCampaigns } from './instantlyRuntime.ts';
import { containCommunicationTransport as realContainCommunicationTransport, persistEmergencyTransportContainment as realPersistEmergencyTransportContainment } from './operationalControl.ts';
import { bootstrapContainedSingleton, readAuthorityRowsForContainment, readSingletonAuthority } from './singletonAuthority.ts';
...
export type EmergencyControlDeps={
  buildEmergencyStopPreview:typeof realBuildEmergencyStopPreview;
  evaluateSafeResume:typeof realEvaluateSafeResume;
  pauseAllInstantlyCampaigns:typeof realPauseAllInstantlyCampaigns;
  containCommunicationTransport:typeof realContainCommunicationTransport;
  persistEmergencyTransportContainment:typeof realPersistEmergencyTransportContainment;
};

const REAL_DEPS:EmergencyControlDeps={
  buildEmergencyStopPreview:realBuildEmergencyStopPreview,
  evaluateSafeResume:realEvaluateSafeResume,
  pauseAllInstantlyCampaigns:realPauseAllInstantlyCampaigns,
  containCommunicationTransport:realContainCommunicationTransport,
  persistEmergencyTransportContainment:realPersistEmergencyTransportContainment,
};
...
export async function handleEmergencyControlAction(user:any,body:any,svc:any,deps:EmergencyControlDeps=REAL_DEPS):Promise<Response>{
  ...
  if(action==='safe_mode_preview'){
    const prepared=await deps.buildEmergencyStopPreview(svc);
  ...
  if(action==='resume_preflight'){
    const preflight=await deps.evaluateSafeResume(svc,body.selected_capabilities);
  ...
  if(action==='safe_mode_on'){
    ...
    for(const transport of ['outlook','resend','instantly'] as const){
      const contained=await deps.containCommunicationTransport(svc,transport,'founder_global_emergency_stop').catch((error:any)=>({ok:false,transport,error:text(error?.code||error?.message,120)}));
    ...
    const remotePause:any=await deps.pauseAllInstantlyCampaigns(svc,'global_emergency_stop').catch((error:any)=>({ok:false,error:text(error?.code||error?.message,120),campaigns:[]}));
  ...
  if(action==='resume_selected'||action==='safe_mode_off'){
    ...
    const preflight=await deps.evaluateSafeResume(svc,selected);
