...
export const RECOVERY_ECONOMICS_COPY_VERSION = 'recover-economics-copy-v4-unified-25-legal-review-per-locale';

type CopyBlock = {
  __translation_readiness?: 'APPROVED' | 'PENDING_LEGAL_REVIEW';
  title: string; term: string; after: string; cambra: string; keep: string;
  referrals: string; verified: string; survival: string; data: string;
};

const EN: CopyBlock = {
  __translation_readiness: 'APPROVED',
  title: '24-month Recovery Term', term: 'Months 1–24', after: 'After month 24',
...
const COPY: Record<string, CopyBlock> = {
  ...
  fr: {
    __translation_readiness: 'APPROVED',
    title: 'Période Recover de 24 mois', term: 'Mois 1 à 24', after: 'Après le 24e mois',
  ...
  es: {
    __translation_readiness: 'APPROVED',
    title: 'Recovery Term de 24 meses', term: 'Meses 1–24', after: 'Después del mes 24',
...
const ACCEPTANCE_EN = (name: string) =>
  `I confirm I can legally bind ${name}. I accept the 24-month Recovery Term shown above: 25% of positive Verified Savings throughout months 1–24, then 0%, subject to applicable referral reductions of 5 percentage points each with a 5% floor during the Recovery Term. I understand that ending other CAMBRA services does not by itself end an already activated Recovery Term and that no fee is due without positive Verified Savings.`;
...
export function recoveryEconomicsAcceptanceText(locale: string, entity: string): string {
  ...
  if (locale === 'fr') {
    return `Je déclare être habilité(e) à engager ${name}. J’accepte la période Recover de 24 mois présentée ci-dessus : 25 % des économies vérifiées positives pendant toute la période des mois 1 à 24, puis 0 %, sous réserve des réductions de parrainage applicables de 5 points chacune avec un plancher de 5 % pendant la période Recover. Je comprends que la fin des autres services CAMBRA ne met pas, à elle seule, fin à une période Recover déjà activée et qu’aucune commission n’est due sans économie positive vérifiée.`;
  }
  if (locale === 'es') {
    return `Declaro que estoy autorizado/a para vincular jurídicamente a ${name}. Acepto el Recovery Term de 24 meses mostrado anteriormente: 25% del ahorro positivo verificado durante todos los meses 1–24 y 0% después, sujeto a descuentos de 5 puntos porcentuales por cada referral aplicable, con un suelo del 5% durante el Recovery Term. Entiendo que cancelar otros servicios de CAMBRA no extingue por sí solo un Recovery Term ya activado y que no existe comisión sin ahorro positivo verificado.`;
  }
