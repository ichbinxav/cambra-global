...
export function benchmarkVisibility(n:number,min=P12_MIN_ANONYMIZED_DISTINCT_MERCHANTS){const threshold=Math.max(P12_MIN_ANONYMIZED_DISTINCT_MERCHANTS,Number.isFinite(min)?min:P12_MIN_ANONYMIZED_DISTINCT_MERCHANTS);return {public:n>=threshold,sample_size:n,precision:n>=20?'high':n>=P12_MIN_ANONYMIZED_DISTINCT_MERCHANTS?'medium':'low'}}
