...
export const GO_CRITICAL_SCHEDULERS = Object.freeze([
  { worker_key:'instantlyProviderEventRetryWorker', cadence_seconds:300, responsibility:'Instantly inbound event recovery and DLQ escalation', owner_system:'Base44', trigger:'scheduled', timeout_seconds:'UNKNOWN', concurrency:'AT_LEAST_ONCE_SLOT_GUARDED', idempotency:'RUN_KEY_AND_PROVIDER_EVENT_KEY', retry_backoff:'1m/5m/15m/60m/240m', dlq_escalation:'OutboundProviderEvent + AutonomyIncident', side_effects:'authenticated provider-event replay', tenant_scope:'platform commercial', authority:'internal secret + service role' },
  { worker_key:'instantlyReconciliationWorker', cadence_seconds:900, responsibility:'bounded Instantly state and missed-event reconciliation', owner_system:'Base44', trigger:'scheduled', timeout_seconds:'UNKNOWN', concurrency:'AT_LEAST_ONCE_SLOT_GUARDED', idempotency:'RUN_KEY_AND_PROVIDER_EVENT_KEY', retry_backoff:'next scheduled run', dlq_escalation:'CommercialProviderState + AutonomyIncident', side_effects:'provider health/profile/event reconciliation', tenant_scope:'platform commercial', authority:'internal secret + service role + cost budget' },
