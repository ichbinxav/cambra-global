...
Deno.serve(async (req) => {let __schedulerSvc:any=null;let __schedulerClaim:any=null;let __schedulerOk=true;
  try {
    ...
    const capabilityBlockedMarkets=new Set(capabilityControls.filter((row:any)=>row.blocked===true&&(!row.effective_to||Date.parse(row.effective_to)>Date.now())).map((row:any)=>String(row.jurisdiction||'').toUpperCase()));
    const policyCountries=[...new Set((Array.isArray(policy.countries)?policy.countries:[]).map((value:any)=>String(value||'').trim().toUpperCase()).filter(Boolean))];
    const scopeBlockedMarkets=policyCountries.filter((country:string)=>!ACTIVE_LAUNCH_MARKET_SET.has(country));
    const blockedMarkets=new Set([...capabilityBlockedMarkets,...scopeBlockedMarkets]);
    ...
    const requestedCountries=policyCountries.filter((value:string)=>ACTIVE_LAUNCH_MARKET_SET.has(value)&&!capabilityBlockedMarkets.has(value)).sort((a:string,b:string)=>(marketPriority.get(a)??1)-(marketPriority.get(b)??1)||a.localeCompare(b));
    const shouldDiscover=!emergency.safe_mode&&!emergency.paid_discovery_paused&&requestedCountries.length>0&&providerOperational&&uniqueCompaniesBefore<targetUnique;
    const discoveryAction=emergency.safe_mode||emergency.paid_discovery_paused?'safe_mode_no_external_discovery':!requestedCountries.length?'no_active_launch_markets':!selectedProvider?providerSelection.reason:uniqueCompaniesBefore>=targetUnique?'warehouse_target_reached':shouldDiscover?'continue_controlled_harvest':'provider_degraded';
    ...
    for(const lead of rankedForDedupe){
      ...
      if(!['contacted','meeting','won'].includes(String(lead.stage))){const duplicateReason=`duplicate_company:${companyWinner.get(domain)}`;if(lead.reservoir_state!=='disqualified'||lead.suppression_reason!==duplicateReason)await service.entities.OutboundLead.update(lead.id,{reservoir_state:'disqualified',suppression_reason:duplicateReason,reservoir_updated_at:now()}).catch((error:any)=>safeBestEffort(error,{operation:'alwaysOnLeadDiscoveryWorker',fallback:null,severity:'secondary'}));duplicateLeadIds.add(lead.id);deduplicated++;}
    }
    ...
    for(const lead of leads){
      ...
      if(suppression.length){suppressed++;if(lead.reservoir_state!=='suppressed'||lead.outreach_eligibility!=='BLOCKED'||lead.suppression_reason!=='contact_suppression')await service.entities.OutboundLead.update(lead.id,{reservoir_state:'suppressed',outreach_eligibility:'BLOCKED',suppression_reason:'contact_suppression',reservoir_updated_at:now()}).catch((error:any)=>safeBestEffort(error,{operation:'alwaysOnLeadDiscoveryWorker',fallback:null,severity:'secondary'}));continue;}
      ...
      if(isReady){outreachReady++;const lastVerifiedAt=Date.parse(lead.last_verified_at||'');const verificationStale=!Number.isFinite(lastVerifiedAt)||lastVerifiedAt<Date.now()-24*3600000;if(lead.reservoir_state!=='ready'||lead.revenue_stage!=='outreach_ready'||!lead.outreach_ready_at||verificationStale)await service.entities.OutboundLead.update(lead.id,{reservoir_state:'ready',revenue_stage:'outreach_ready',outreach_ready_at:lead.outreach_ready_at||now(),last_verified_at:now(),reservoir_updated_at:now()}).catch((error:any)=>safeBestEffort(error,{operation:'alwaysOnLeadDiscoveryWorker',fallback:null,severity:'secondary'}));}
      else if(['lead','enriched','scored'].includes(String(lead.stage))){const desiredReservoirState=lead.stage==='lead'?'discovered':lead.stage==='enriched'?'enriching':'qualified';if(lead.reservoir_state!==desiredReservoirState)await service.entities.OutboundLead.update(lead.id,{reservoir_state:desiredReservoirState,reservoir_updated_at:now()}).catch((error:any)=>safeBestEffort(error,{operation:'alwaysOnLeadDiscoveryWorker',fallback:null,severity:'secondary'}));}
