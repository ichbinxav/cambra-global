...
async function gh(token: string, path: string) {
  ...
  if (!r.ok)
    throw new Error(
      `github_${r.status}:${typeof data === "string" ? data.slice(0, 160) : data?.message || "request_failed"}`,
    );
  return data;
}
async function incident(
  ...
  const old =
    (
      await s.entities.AutonomyIncident.filter(
        { dedupe_key: key, status: "open" },
        "-last_seen_at",
        1,
      ).catch((error: any) =>
        safeBestEffort(error, {
          operation: "dependencySecurityWorker",
          fallback: [],
          severity: "secondary",
        }),
      )
  ...
  const row = {
    ...
    customer_impact: "low",
    legal_risk: "high",
    subject_type: "DeveloperWorkspace",
    subject_id: String(details?.workspace_id || "_platform"),
    summary,
    details_json: { ...details, source: "github_dependabot" },
    first_seen_at: old?.first_seen_at || now(),
    last_seen_at: now(),
  };
  if (old) {
    await s.entities.AutonomyIncident.update(old.id, row);
    return old.id;
  }
  return (await s.entities.AutonomyIncident.create(row)).id;
}
guardedScheduledServe(
  { worker_key: "dependencySecurityWorker", cadence_seconds: 21600 },
  ...
  async (req) => {
    try {
      ...
      const s = b.asServiceRole,
        ...
        workspaces = await s.entities.DeveloperWorkspace.list(
          "-updated_date",
          500,
        ).catch((error: any) =>
          safeBestEffort(error, {
      ...
      if (!conn?.accessToken) {
        await incident(
          s,
          "p17:dependency_monitor:github_unavailable",
          "Dependency vulnerability watch cannot query GitHub because the connector is unavailable.",
          {
            workspace_id: "_platform",
            registered_workspaces: workspaces.length,
          },
          "warning",
        );
        return Response.json(
          {
            ok: false,
            error: "github_connector_required",
            registered_workspaces: workspaces.length,
          },
          { status: 409 },
        );
      ...
      for (const w of workspaces) {
        ...
        try {
            ...
            const sev = String(
              ...
              num = String(
                a?.number ||
                  ...
                  `${a?.dependency?.package?.name || "unknown"}:${a?.dependency?.manifest_path || "unknown"}`,
              ),
              key = `p17:dependency:${repo}:${num}`,
              isCritical = ["critical", "high"].includes(sev);
            activeKeys.add(key);
            await incident(
              s,
              key,
              `Open ${sev} dependency vulnerability in ${repo}: ${a?.security_advisory?.summary || a?.dependency?.package?.name || "dependency alert"}`,
              {
                workspace_id: w.id,
                repo_full_name: repo,
                alert_number: a?.number || null,
          ...
          await incident(
            s,
            key,
            `Dependency vulnerability watch failed for ${repo}`,
            {
              workspace_id: w.id,
              repo_full_name: repo,
      ...
      const previous = await s.entities.AutonomyIncident.filter(
        { domain: "security", status: "open" },
        "-last_seen_at",
        500,
      ).catch((error: any) =>
        safeBestEffort(error, {
      ...
      for (const i of previous.filter(
        ...
        await s.entities.AutonomyIncident.update(i.id, {
          ...
          recovery_json: {
            ...
            verified: true,
            note: "Alert or monitor failure no longer returned by the authoritative GitHub sweep.",
          },
        }).catch((error: any) =>
          safeBestEffort(error, {
            operation: "dependencySecurityWorker",
      ...
      console.error(e);
      return internalErrorResponse(e, "dependencySecurityWorker");
    }
  },
);
