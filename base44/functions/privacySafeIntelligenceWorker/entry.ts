...
async function upsert(s:any,key:string,payload:any){
 const old=(await s.entities.AnonymizedIntelligenceAggregate.filter({aggregate_key:key},'-created_date',1).catch((error:any)=>safeBestEffort(error,{operation:'privacySafeIntelligenceWorker',fallback:[],severity:'secondary'})))[0];
...
guardedScheduledServe({"worker_key":"privacySafeIntelligenceWorker","cadence_seconds":86400},createClientFromRequest,async req=>{let s:any=null,evidence:any=null,start:any=null;try{
 ...
 evidence=await s.entities.RetentionExecutionEvidence.create(start.row).catch((error:any)=>safeBestEffort(error,{operation:'privacySafeIntelligenceWorker',fallback:null,severity:'secondary'}));if(!evidence)return Response.json({ok:false,error:'retention_audit_evidence_unavailable'},{status:503});
 const cohorts=await s.entities.BenchmarkCohort.list('-month',5000).catch((error:any)=>safeBestEffort(error,{operation:'privacySafeIntelligenceWorker',fallback:[],severity:'secondary'})),outcomes=(await s.entities.IntelligenceOutcome.list('-captured_at',5000).catch((error:any)=>safeBestEffort(error,{operation:'privacySafeIntelligenceWorker',fallback:[],severity:'secondary'}))).filter((x:any)=>!x.is_demo);let created=0,updated=0,suppressed=0;
 ...
 await s.entities.OperationalLog.create({event_type:'intelligence_event',message:'privacy_safe_intelligence_refresh',data_json:{created,updated,suppressed,version:PRIVACY_SAFE_INTELLIGENCE_VERSION,retention_evidence_id:evidence.id},actor_email:'internal',created_at:new Date().toISOString()}).catch((error:any)=>safeBestEffort(error,{operation:'privacySafeIntelligenceWorker',fallback:null,severity:'secondary'}));
...
}catch(e){console.error(e);if(s&&evidence?.id){await s.entities.RetentionExecutionEvidence.update(evidence.id,{status:'FAILED',completed_at:new Date().toISOString(),failed_count:1,evidence_json:{raw_identifiers_persisted:false,error_code:'privacy_safe_intelligence_failed'}}).catch((error:any)=>safeBestEffort(error,{operation:'privacySafeIntelligenceWorker',fallback:null,severity:'secondary'}))}return internalErrorResponse(e, 'privacySafeIntelligenceWorker');}});
