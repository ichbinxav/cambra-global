...
Deno.serve(async(req)=>{
  try{
    ...
    const obs=await s.entities.PaymentRoutingObservation.filter({data_classification:'production',learning_eligible:true,is_demo:false},'-created_date',5000).catch(()=>[]);
    const brands=await s.entities.Brand.list('-created_date',2000).catch(()=>[]);
    const routingOpp=await s.entities.RoutingOpportunity.filter({status:{$in:['candidate','qualified','recommended']},is_demo:false},'-created_at',3000).catch(()=>[]);
    const recover=await s.entities.DealActivation.filter({vertical:'payments',status:{$in:['authorized','migrating','live','monetizing']}},'-last_updated',3000).catch(()=>[]);
    const commitments=await s.entities.AggregateCommitment.filter({status:{$in:['explicitly_authorized','active']}},'-created_at',3000).catch(()=>[]);
    ...
    for(const [brandId,rows] of byBrand){
      ...
      for(const [key,rs] of groups){
        ...
        const old=await s.entities.DemandUnit.filter({demand_key:demandKey},'-created_date',1).catch(()=>[]); let saved:any;
    ...
    for(const [key,rows] of poolGroups){
      ...
      const old=await s.entities.AggregatePool.filter({pool_key:key},'-created_date',1).catch(()=>[]); const merchantCount=new Set(rows.map((x:any)=>x.brand_id)).size;
