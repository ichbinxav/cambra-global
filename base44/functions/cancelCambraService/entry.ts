...
export default async function(req:Request):Promise<Response>{
 try{
  ...
  const acts=await svc.entities.DealActivation.filter({brand_id:brand.id},'-created_date',100).catch(()=>[]); const surviving=[]; const paused=[];
  for(const a of acts){
    ...
    } else if(a.status){ await svc.entities.DealActivation.update(a.id,{service_cancelled_at:now}).catch(()=>null); }
  }
  await svc.entities.OperationalLog.create({brand_id:brand.id,event_type:'status_changed',message:'cambra_service_cancelled',data_json:{surviving_recoveries:surviving,paused_unactivated_recoveries:paused,recovery_terms_acknowledged:true},actor_email:user.email,created_at:now}).catch(()=>null);
