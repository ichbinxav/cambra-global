...
Deno.serve(async (req) => {
  ...
  // ——— Savings tracking: persist BrandSavings from AnalyzerResult (single source of truth) ———
  // DO NOT recalculate here. AnalyzerResult already contains savings computed by scoreEngine.js
  // with all caps, tier logic, and proportional scaling applied. Reading from it ensures
  // BrandSavings, emails, dashboard, and billing all show the same figures.
  try {
    const brandId = data.brand_id || null;
    const savingsYearly  = Number(data.total_savings    || 0);
    const savingsMonthly = Number((savingsYearly / 12).toFixed(2));

    await base44.asServiceRole.entities.BrandSavings.create({
      brand_id: brandId || "",
      result_id: data.id,
      // Breakdowns also read directly from AnalyzerResult — no independent formula
      payment_savings:  Number(data.payment_savings  || 0),
      shipping_savings: Number(data.shipping_savings || 0),
      saas_savings:     Number(data.saas_savings     || 0),
  ...
  await base44.asServiceRole.integrations.Core.SendEmail({
    ...
    body: `
      <div style="font-family: -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 32px; color: #111;">
        <p style="font-size: 11px; letter-spacing: 0.2em; text-transform: uppercase; color: #999; margin-bottom: 32px;">CAMBRA · Infrastructure Analyzer</p>
        ...
        <a href="https://${appDomain}/Results?id=${resultId}" style="display: inline-block; background: #111; color: #fff; text-decoration: none; font-weight: 700; font-size: 14px; padding: 14px 28px; border-radius: 100px; margin-right: 12px;">
          View full report →
        </a>
        <a href="https://${appDomain}/Deals" style="display: inline-block; color: #111; text-decoration: none; font-weight: 600; font-size: 14px; padding: 14px 0;">
          Activate deals →
        </a>
        ...
        <div style="margin-top: 48px; padding-top: 24px; border-top: 1px solid #eee;">
          ...
          <p style="font-size: 11px; color: #ccc; margin-top: 4px;">CAMBRA · Infrastructure Intelligence for independent brands</p>
        </div>
      </div>
    `,
  });
