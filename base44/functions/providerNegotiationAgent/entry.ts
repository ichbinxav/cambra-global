  ...
  try {
    ...
    if (body?.action === 'initial_contact') {
      ...
      const prompt = [
        ...
        'Use only facts supplied. Do not mention merchant identities or confidential competitor offers. You may use anonymized aggregate leverage, but NEVER describe addressable/observed volume as committed or guaranteed. Request either one preferred rate or, if unavailable, an indicative cohort/tier matrix segmented by country, channel, sector/MCC, monthly or annual volume, average ticket and domestic/EEA/international/commercial-card mix. State that every rate remains subject to provider underwriting and request all components needed for like-for-like comparison.',
    ...
    const prompt = [
      ...
      'Do not disclose competitor confidential quotes. Do not disclose the merchant identity unless authority.disclosable explicitly contains merchant_identity; otherwise keep the case anonymized. If an explicit target exists, push toward it. If NO explicit target exists, DO NOT invent a percentage, price or numerical anchor: ask for a stronger best-and-final improvement and clarify missing commercial components. Accept either one preferred rate or an indicative cohort/tier matrix by country, channel, sector/MCC, volume, average ticket and card mix, always subject to provider underwriting.',
      'Comparable outcomes are a private descriptive aggregate heuristic only, not statistical or probabilistic calibration: never quote them as a probability, public provider rate, target, promise, guarantee or authority to accept terms. A suppressed advisory provides no usable signal.',
