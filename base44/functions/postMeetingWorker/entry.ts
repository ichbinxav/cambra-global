...
async function requestOutcomeCapture(svc:any, thread:any) {
  const tasks=await svc.entities.AgentTask.filter({agent_name:'post_meeting_worker',task_type:'capture_founder_meeting_outcome',related_entity_id:thread.id,status:'waiting_input'},'-created_date',1).catch(()=>[]);
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const rows=await svc.entities.CommunicationThread.filter({post_meeting_status:'pending',meeting_end_at:{$lte:now.toISOString()}},'meeting_end_at',50).catch(()=>[]);
    ...
    for(const thread of rows){
      ...
      const suppression=await svc.entities.ContactSuppression.filter({email:thread.counterparty_email,active:true},'-created_date',1).catch(()=>[]);
      ...
      const policies=await svc.entities.CommercialPolicy.filter({policy_key:thread.policy_key,status:'active'},'-approved_at',5).catch(()=>[]);
      ...
      const messages=await svc.entities.CommunicationMessage.filter({thread_id:thread.id},'created_date',30).catch(()=>[]);
      const prompt=[
      ...
      await svc.entities.OperationalLog.create({event_type:'AI_NEGOTIATION_RESUMED',message:'CAMBRA resumed after founder meeting',data_json:{thread_id:thread.id,outcome:finalState,send_id:data.message_id||null},created_at:new Date().toISOString()}).catch(()=>null);
      sent++;
    }
