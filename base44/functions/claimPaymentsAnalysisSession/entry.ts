...
//
// ── FULL REWRITE 2026-07-13 (deploy-unstick) ────────────────────────────────
// Diagnosis: the DEPLOYED build of this function was frozen on an intermediate
// version whose `details:` block wrote the LEGACY scoreEngine shape
// (shipping/saas/banking) instead of the payments shape (engine_result +
// input_snapshot), which left /Results blank for owned rows. Incremental
// find_replace edits did not take effect in runtime. This is a FULL-FILE
// rewrite (write_file, not find_replace) to force a clean build. The
// details_shape:"payments-v1" fingerprint below is the deploy probe — once a
// freshly-claimed row carries it, the deploy has landed.
...
Deno.serve(async (req) => {
  try {
    ...
    const created = await base44.entities.AnalyzerResult.create({
      ...
      details: {
        // DEPLOY FINGERPRINT 2026-07-13 — proves this build is live once a
        // freshly-claimed row carries it. The payments shape below is what
        // /Results reads (engine_result + input_snapshot); the frozen build
        // was writing a legacy scoreEngine shape instead.
