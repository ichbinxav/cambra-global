...
async function runInstantlyPreviewDiscovery(service:any,body:any){
  ...
  const states=await service.entities.CommercialProviderState.filter({provider_key:'instantly_supersearch',role:'lead_intelligence'},'-last_checked_at',1).catch(()=>[]);
