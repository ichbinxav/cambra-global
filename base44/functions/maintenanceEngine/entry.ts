...
import { integrationHealthScope } from '../../shared/integrationHealth.ts';
import { maintenanceIncidentAbsenceResolution, staleTaskIncidentSubjectId } from '../../shared/maintenanceIncidentLifecycle.ts';
...
async function resolveReconciledIncident(s:any,incident:any,evidence:any){
 const at=now(),priorActions=Array.isArray(incident?.actions_taken_json)?incident.actions_taken_json:[];
 await s.entities.AutonomyIncident.update(incident.id,{status:'resolved',workflow_state:'resolved',resolved_at:at,actions_taken_json:[...priorActions,{at,action:'reconcile_current_signal_state',verified:true,evidence}].slice(-100),recovery_json:{...(incident?.recovery_json||{}),verified:true,reconciliation:evidence}});
}
async function reconcileInactiveIncidents(s:any,signals:MaintenanceSignal[]){
 const activeKeys=new Set(signals.map((signal)=>signal.key));
 const open=await s.entities.AutonomyIncident.filter({status:'open'},'-last_seen_at',500);
 let resolvedByAbsence=0,resolvedTerminalTasks=0,unverifiedTaskEvidence=0;
 for(const incident of open){
  const absence=maintenanceIncidentAbsenceResolution(incident,activeKeys);
  if(absence){await resolveReconciledIncident(s,incident,absence);resolvedByAbsence++;continue}
  const taskId=staleTaskIncidentSubjectId(incident);if(!taskId)continue;
  const task=await s.entities.AgentTask.get(taskId).catch((error:any)=>safeBestEffort(error,{operation:'maintenanceEngine.reconcileStaleTask',fallback:null,severity:'secondary'}));
  if(!task){unverifiedTaskEvidence++;continue}
  if(task.status==='running')continue;
  await resolveReconciledIncident(s,incident,{reason:'agent_task_readback_is_terminal',task_id:task.id,task_status:task.status,completed_at:task.completed_at||null});resolvedTerminalTasks++;
 }
 return{inspected:open.length,resolved_by_absence:resolvedByAbsence,resolved_terminal_tasks:resolvedTerminalTasks,unverified_task_evidence:unverifiedTaskEvidence};
}
...
if(routed.host_action==='cost_governance')return (await handleCostGovernanceWorker(req))||Response.json({ok:false,error:'cost_governance_response_missing'},{status:500});if(routed.host_action==='production_readiness')return (await handleProductionReadinessWorker(req))||Response.json({ok:false,error:'production_readiness_response_missing'},{status:500});if(String(routed.host_action||'').startsWith('disaster_recovery_backup')||String(routed.action||'').startsWith('dr_'))return (await handleDisasterRecovery(req))||Response.json({ok:false,error:'disaster_recovery_response_missing'},{status:500});const b=createClientFromRequest(req);const body=await req.json().catch(()=>({}));const gate=await requireAdminOrInternal(req,b,body);if(!gate.ok)return gate.response||Response.json({error:'forbidden'},{status:403});const s=b.asServiceRole,started=now(),run=await s.entities.MaintenanceRun.create({run_key:`maintenance:${started}`,engine_version:MAINTENANCE_VERSION,status:'running',started_at:started});try{
 const [integrations,deadLetters,tasks,pricing,invoices,securityAudits,providerStatements,outboundControls,disasterRecoveryEvents]=await Promise.all([
  s.entities.Integration.list('-created_date',2000),s.entities.WebhookDeadLetter.list('-created_date',1000),s.entities.AgentTask.list('-created_date',5000),s.entities.ProviderPricingVersion.list('-observed_at',2000),s.entities.Invoice.list('-created_date',3000),s.entities.SecurityAudit.list('-created_date',1000),s.entities.ProviderRevenueStatement.list('-received_at',1000),s.entities.OutboundControl.filter({control_key:'global'},'-created_date',2),s.entities.OperationalLog.filter({event_type:{$in:['disaster_recovery_backup_completed','disaster_recovery_backup_failed']}},'-created_at',50).catch((error:any)=>safeBestEffort(error,{operation:'maintenanceEngine',fallback:[],severity:'secondary'}))
 ]);
 ...
 const schedulerHealth=await readCriticalSchedulerEvidence(s,t,disasterRecoveryCompletionEvidence(disasterRecoveryEvents));for(const row of schedulerHealth.rows.filter((item:any)=>item.status!=='HEALTHY'))signals.push({key:`final:scheduler:${row.worker_key}:${String(row.status).toLowerCase()}`,domain:'worker',type:`scheduler_${String(row.status).toLowerCase()}`,severity:row.status==='FAILED'?'critical':'warning',subjectType:'SchedulerRun',subjectId:row.worker_key,summary:`GO-critical scheduler ${row.worker_key} is ${row.status}`,customerImpact:row.status==='FAILED'?'high':'medium',details:{status:row.status,cadence_seconds:row.cadence_seconds,age_seconds:row.age_seconds,duplicate_executions:row.duplicate_executions,responsibility:row.responsibility,evidence_coverage:row.evidence_coverage,root_cause:'runtime_scheduler_evidence_not_healthy'}});
 ...
 for(const i of integrations){
  if(!integrationHealthScope(i).included)continue;
 ...
 const reservoir=(await s.entities.LeadReservoirSnapshot.list('-captured_at',2))[0]||null,outboundExpected=outboundControls[0]?.acquisition_enabled===true;if(outboundExpected&&(!reservoir||t-Date.parse(reservoir.captured_at||'')>3*HOUR))signals.push({key:'p19:lead_discovery_worker_stale',domain:'worker',type:'lead_discovery_worker_stopped',severity:'warning',subjectType:'LeadReservoirSnapshot',subjectId:reservoir?.id||'_missing',summary:'Always-on lead discovery has no fresh reservoir snapshot while outbound acquisition is enabled',details:{last_snapshot_at:reservoir?.captured_at||null,outbound_expected:true}});if(reservoir?.coverage_status==='LOW'&&Number(reservoir?.safe_daily_send_capacity||0)>0)signals.push({key:'p19:qualified_reservoir_low',domain:'worker',type:'qualified_reservoir_low',severity:'warning',subjectType:'LeadReservoirSnapshot',subjectId:reservoir.id,summary:`Qualified lead reservoir coverage is low (${reservoir.coverage_days} days)`,details:{coverage_days:reservoir.coverage_days,target_coverage_days:reservoir.target_coverage_days,outreach_ready:reservoir.outreach_ready,safe_daily_send_capacity:reservoir.safe_daily_send_capacity}});if(Number(reservoir?.stale||0)>Math.max(25,Number(reservoir?.outreach_ready||0)))signals.push({key:'p19:lead_staleness_high',domain:'intelligence',type:'lead_staleness_high',severity:'warning',subjectType:'LeadReservoirSnapshot',subjectId:reservoir.id,summary:'Stale lead inventory is elevated',details:{stale:reservoir.stale,outreach_ready:reservoir.outreach_ready}});
