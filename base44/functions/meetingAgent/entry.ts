...
Deno.serve(async(req)=>{
  try{
    ...
    if(body.mode==='execute'){
      const approval=await svc.entities.Approval.get(String(body.approval_id||'')).catch(()=>null);
    ...
    const lead=await svc.entities.OutboundLead.get(String(body.lead_id||'')).catch(()=>null);
    if(!lead)return Response.json({ok:false,error:'lead_required'},{status:404});
    const threads=await svc.entities.CommunicationThread.filter({lead_id:lead.id,engine:'merchant_acquisition'},'-created_date',10).catch(()=>[]);
  ...
  }catch(error){console.error('meetingAgent failed',error);return Response.json({ok:false,error:'meeting_agent_failed',detail:String(error?.message||error)},{status:500});}
});
