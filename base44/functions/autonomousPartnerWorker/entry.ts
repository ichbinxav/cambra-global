...
async function claudeDraft(svc:any, prospect:any, language:string, variant:any, eventKey:string){
 ...
 const out=await callCambraClaude(prompt,{tier:'standard',maxTokens:850,svc,eventKey,source:'autonomousPartnerWorker'});const t=String(out.text||'').replace(/```json\s*/gi,'').replace(/```/g,'').trim();try{return JSON.parse(t)}catch{const m=t.match(/\{[\s\S]*\}/);if(m)try{return JSON.parse(m[0])}catch(error){safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:null,severity:'secondary'})}return null}
}

Deno.serve(async(req)=>{let task:any=null;let schedulerSvc:any=null;let schedulerClaim:any=null;let schedulerOk=true;try{
 const base44=createClientFromRequest(req);const body=await req.clone().json().catch(()=>({}));const gate=await requireAdminOrInternal(req,base44,body);if(!gate.ok)return gate.response;const svc=base44.asServiceRole;schedulerSvc=svc;schedulerClaim=await claimSchedulerRun(svc,req,{worker_key:'autonomousPartnerWorker',cadence_seconds:3600});{const denied=schedulerClaimDeniedResponse(schedulerClaim);if(denied)return denied;}schedulerClaim=await markSchedulerEffectStarted(svc,schedulerClaim);{const denied=schedulerClaimDeniedResponse(schedulerClaim);if(denied)return denied;}
 ...
 const controls=await svc.entities.OutboundControl.filter({control_key:'global'},'-created_date',1).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));const control=controls[0]||null;if(!control?.acquisition_enabled||!control?.premium_outlook_enabled)return Response.json({ok:true,automatic:false,reason:'outbound_paused'});
 ...
 const day=new Date();day.setUTCHours(0,0,0,0);const sentToday=await svc.entities.CommunicationMessage.filter({direction:'outbound',policy_key:policy.policy_key,sent_at:{$gte:day.toISOString()}},'-sent_at',Math.min(Number(policy.daily_send_limit||20)+5,100)).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));let remaining=Math.max(0,Number(policy.daily_send_limit||20)-sentToday.length);if(!remaining)return Response.json({ok:true,automatic:true,contacted:0,reason:'daily_limit'});
 ...
 const expStats=await svc.entities.OutreachExperimentStats.filter({engine:'partner_acquisition'},'-updated_at',50).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));
 const priorPartners=await svc.entities.PartnerProspect.filter({stage:{$in:['contacted','replied','meeting','qualified','won']}},'-created_date',1000).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));const contactedOrgDomains=new Set(priorPartners.map((p:any)=>String(p.organization_domain||'').replace(/^https?:\/\//,'').replace(/^www\./,'').split('/')[0].toLowerCase()).filter(Boolean));const seenOrgDomains=new Set<string>();
 ...
 for(const countryCode of countries){const market=canonicalMarket(countryCode);if(!market){failures.push({country:String(countryCode),error:'market_not_canonical'});continue}const country=market.canonical_name;const people=await apollo(svc,apolloKey,country,titles,per).catch((e:any)=>{failures.push({country,error:String(e?.message||e)});return[]});discovered+=people.length;
  for(const person of people){if(contacted>=remaining)break;const email=normalizeEmail(person?.email);if(!email){skipped++;continue}const org=person?.organization||{};const orgName=String(org.name||'').trim();if(!orgName){skipped++;continue}const domain=String(org.primary_domain||org.website_url||email.split('@')[1]||'').replace(/^https?:\/\//,'').replace(/\/.*$/,'').toLowerCase();if(!domain||seenOrgDomains.has(domain)||contactedOrgDomains.has(domain)){skipped++;continue}seenOrgDomains.add(domain);
   const suppressed=await svc.entities.ContactSuppression.filter({email,active:true},'-created_date',1).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));if(suppressed.length){skipped++;continue}
   const existing=await svc.entities.PartnerProspect.filter({contact_email:email},'-created_date',5).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));let prospect=existing[0]||null;
   ...
   const threads=await svc.entities.CommunicationThread.filter({thread_key:`partner:${prospect.id}`},'-created_date',2).catch((error:any)=>safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:[],severity:'secondary'}));if(threads.length){skipped++;continue}
...
}catch(error){schedulerOk=false;console.error('autonomousPartnerWorker failed',error);if(task?.id){try{const b=createClientFromRequest(req);await b.asServiceRole.entities.AgentTask.update(task.id,{status:'failed',error:String(error?.message||error),completed_at:new Date().toISOString()})}catch(error){safeBestEffort(error,{operation:'autonomousPartnerWorker',fallback:null,severity:'secondary'})}}return internalErrorResponse(error, 'autonomousPartnerWorker');}finally{if(schedulerSvc&&schedulerClaim?.allowed===true)await finishSchedulerRunOrThrow(schedulerSvc,schedulerClaim,{worker_key:'autonomousPartnerWorker'},schedulerOk)}});
