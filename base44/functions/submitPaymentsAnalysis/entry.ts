...
const ALLOWED_PROVIDER_SLUGS = [
  ...
  'checkout_com',
  'sumup',
  // 0.2b (2026-07-13) — FR online PSPs with a seeded <slug>|ANY|EU row.
  'payplug',
  'stancer',
  'lyra',
...
Deno.serve(async (req) => {
  try {
    ...
    if (isCombined) {
      ...
      for (const c of cleanChannels) {
        const engineInput = {
          ...
          region,
          // M5 — country preference for row selection (field-based, never
          // parsed from cohort_key). Same country for both channels.
          country,
    ...
    const engineInput = {
      ...
      // M5 — country preference for row selection (field-based, never parsed
      // from cohort_key). With no country rows seeded, behavior is identical.
      country: v.clean.country,
