...
export default async function (req: Request): Promise<Response> {
  try {
    ...
    await svc.entities.DealActivation.update(activation.id, {
      ...
      ...(isV2 ? { recovery_economics_version: RECOVERY_ECONOMICS_V2, recovery_term_start_date: v2Term.start, recovery_term_end_date: v2Term.endExclusive, economic_right_status: 'active', verification_access_status: 'connected', recovery_attribution_key: attributionKey, recovery_mandate_id: activeMandate.id } : {}),
    });
    ...
    await svc.entities.OperationalLog.create({
      ...
      data_json: {
        ...
        ...(isV2 ? { recovery_economics_version: RECOVERY_ECONOMICS_V2, recovery_term_start_date: v2Term.start, recovery_term_end_date: v2Term.endExclusive, economic_right_status: 'active', recovery_attribution_key: attributionKey, recovery_mandate_id: activeMandate.id } : {}),
        source,
    ...
    return Response.json({
      ...
      first_invoice_eligible_at: firstInvoiceEligible,
      agreement_end_at: isV2 ? v2Term.endExclusive : endAt,
      ...(isV2 ? { recovery_term_start_date: v2Term.start, recovery_term_end_date: v2Term.endExclusive, economic_right_status: 'active', recovery_attribution_key: attributionKey, recovery_mandate_id: activeMandate.id } : {}),
    });
  } catch (error) {
