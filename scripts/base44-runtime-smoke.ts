...
await positive('Discovery V2 zero-cost plan','adminSummaries',{ action:'discovery_v2_plan',discovery_type:'MERCHANT',source_mode:'CAMBRA',target_count:25,hard_cap_minor:0,enrichment_policy:'NONE',filters:{ country:['ES'],industry:['ecommerce'] } },(data)=>data?.ok===true&&data?.plan?.selected_source==='CAMBRA'&&data?.plan?.cost?.estimated_minor===0&&data?.plan?.outbound_effect==='NONE');
