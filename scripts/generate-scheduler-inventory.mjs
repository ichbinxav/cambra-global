...
const dirs=fs.readdirSync('base44/functions',{withFileTypes:true}).filter((entry)=>entry.isDirectory()).map((entry)=>entry.name).sort();
const allFunctionSources=[];for(const dir of dirs)for(const name of fs.readdirSync(path.join('base44/functions',dir)))if(name.endsWith('.ts'))allFunctionSources.push(fs.readFileSync(path.join('base44/functions',dir,name),'utf8'));
const rows=[];
for(const dir of dirs){
  ...
  for(const [index,automation] of (config.automations||[]).entries()){
    ...
    const workerKey=String(automation.function_name||config.name||dir);
